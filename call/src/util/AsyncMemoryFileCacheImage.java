...
import util.DiskLruCache.Snapshot;
...
/**
 * 解决 类似 ListView 与 GridView 等视图组件在使用AsyncTask异步加载图片滚动时出现图片错乱问题， 
 * AsyncTask异步多线程技术
 * 内存缓存实现LRUCache
 * 文件缓存DiskCache
 * @author Viktor.zhou
 * 
 */
public class AsyncMemoryFileCacheImage {
	...
	private static AsyncMemoryFileCacheImage mAsycnHttpImageView = null;

	public static AsyncMemoryFileCacheImage getInstance(Context context) {
		if (mAsycnHttpImageView == null) {
			mAsycnHttpImageView = new AsyncMemoryFileCacheImage(context);
		}
		return mAsycnHttpImageView;
	}

	private AsyncMemoryFileCacheImage(Context context) {
		// 获取应用程序最大可用内存
	 ...
	 * @param imageView
	 * widht height 默认为 0
	 *  R.drawable.transparent：  在res/values文件夹下定义drawables.xml文件，设置如下值：表示默认图片透明
	...
	public void loadBitmap(Resources res, String imageUrl, ImageView imageView) {
		...
		if (cancelPotentialWork(imageUrl, imageView)) {
			...
			AsyncDrawable asyncDrawable = new AsyncDrawable(res,BitmapFactory.decodeResource(res, R.drawable.img_smile), task);
			imageView.setImageDrawable(asyncDrawable);
